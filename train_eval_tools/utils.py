...
class FullModel(nn.Module):
    ...
    def forward(self, inputs, labels, bd_gt, *args, **kwargs):
        ...
        loss_1 = self.sem_loss(outputs[0], labels)
        loss_2 = self.sem_loss(outputs[2], labels)
        loss_3 = self.sem_loss(outputs[1], labels)
        # 最终损失
        loss = loss_3 + 0.1 * loss_1 + 0.1 * loss_2

        return torch.unsqueeze(loss, 0), outputs[1], acc, [loss_1, loss_2]
